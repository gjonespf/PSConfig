...
Function Get-ConfigurationItem {
    ...
    were loaded.  Alternatively, if override is enabled we search for all configuration sources and supply the last result.

    .PARAMETER Key
    Specifies the key to retrieve a value for.

    .PARAMETER Override
    Switch flag, if used will continue to search for items and return the last result.
